...
class AlienContact(BaseModel):
    contact_id: str = Field(..., min_length=5, max_length=15, description="Contact identifier")
    timestamp: datetime = Field(..., description="Contact timestamp")
    location: str = Field(..., min_length=3, max_length=100, description="Contact location")
    contact_type: ContactType = Field(..., description="Type of contact")
    signal_strength: float = Field(..., ge=0.0, le=10.0, description="Signal strength 0-10")
    duration_minutes: int = Field(..., ge=1, le=1440, description="Duration in minutes (max 24h)")
    witness_count: int = Field(..., ge=1, le=100, description="Number of witnesses")
    message_received: Optional[str] = Field(default=None, max_length=500, description="Received message")
